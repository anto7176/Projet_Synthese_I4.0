...
def normaliser_qualite(df_y, colonne='quality'):
    """
    Normalise la colonne quality entre 0 et 100.
    Formule : quality_norm = (quality - min) / (max - min) * 100
    """
    if df_y is None:
        return None
    ...
    print(f"--- Normalisation de '{colonne}' ---")
    print(f"Valeur min originale : {q_min:.4f}  →  0.0")
    print(f"Valeur max originale : {q_max:.4f}  →  100.0")
    print(f"Nouvelle plage : [{df_norm[colonne].min():.1f}, {df_norm[colonne].max():.1f}]")
    print(f"--------------------------\n")
...
if __name__ == "__main__":
    ...
    if df_x is not None and df_y is not None:
        ...
        # Normalisation de quality entre 0 et 100
        ...
        print("\nStatistiques quality normalisée :")
